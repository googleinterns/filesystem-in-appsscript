 ...
 * 
 * @param {String} localPath The local file path whose mapped file is to be deleted
...
function deleteFile(localPath) {
  ...
  if(exists) {
    ...
    throw new FileDoesNotExistException("File Mapped to the local path provided has already been deleted.");
  }
  return exists;
}

/**
 * API to delete the drive folder which is mapped to the absolute local folder path.
 * 
 * @param {String} localPath The local folder path whose mapped folder is to be deleted
...
function deleteFolder(localPath) {
  ...
  if(exists) {
    ...
    // To delete all the mappings in the config which has this localPath as a part of their path
    var documentProperties = PropertiesService.getDocumentProperties();
    var properties = documentProperties.getProperties();
    
    for (var property in properties) {
      // startsWith() is case sensitive - need to take care of this
      if(property.startsWith(localPath)){
    ...
    throw new FolderDoesNotExistException("Folder Mapped to the local path provided has already been deleted.");
  }
  return exists;
}
