...
/**
 * API to get all the file and folder names which matches a specified pattern. It supports the use of multiple
 * character (*) and single character (?) wildcards to specify multiple files and folders.
 * 
 * @param {String} pattern The Windows or Unix pattern containing wildcards like '*' and '?'
 * @return {StringArray} matches The names of files, directories, or folders that matches the specified pattern.
 */
function getPatternMatches(pattern) {
   ...
   * files/folders are to be found by making a call to the getFolderId() function
   * if it return a drive id then find all the files/folders matching the given pattern within
   * this folder using regular expressions and return the array of all file/folder names
   */
  if(hasFolder(directoryPath)){
    ...
    while(files.hasNext()){
      ...
      if(tempArray.length > 0 && tempArray[0] === file.getName()){
